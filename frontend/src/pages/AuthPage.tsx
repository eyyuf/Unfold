...
export function AuthPage({
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<AuthFields>({ resolver: zodResolver(authSchema(mode)) })
  ...
  return (
    ...
    >
      <Card className="auth-card" style={{ width: '100%', maxWidth: 420, padding: 32 }}>
        ...
        <p style={{ color: C.t3, marginBottom: 24 }}>
          {mode === 'login'
            ? 'Continue your active experiment.'
            : 'Start collecting evidence from real experience.'}
        </p>
        <form onSubmit={handleSubmit((values) => mutation.mutate(values))}>
          {(['email', 'password'] as const).map((field) => (
            <label
              key={field}
              style={{ display: 'block', color: C.t2, fontSize: 14, marginBottom: 18 }}
            >
              ...
              <input
                ...
                style={{
                  ...
                  background: C.s2,
                  color: C.t1,
                  border: `1px solid ${errors[field] ? C.red : C.br}`,
                  borderRadius: 10,
                  padding: '12px 14px',
                  font: 'inherit',
                }}
              />
              {errors[field] && (
                <span style={{ color: C.red, display: 'block', marginTop: 5, fontSize: 12 }}>
                      ...
                      {[1, 2, 3, 4].map((i) => (
                        <div
                          ...
                          style={{
                            ...
                            height: 3,
                            borderRadius: 2,
                            background: i <= strength ? colors[strength] : C.s2,
          ...
          {mode === 'register' && (
            <>
              <label style={{ display: 'block', color: C.t2, fontSize: 14, marginBottom: 18 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    background: C.s2,
                    color: C.t1,
                    border: `1px solid ${errors.confirm_password ? C.red : C.br}`,
                    borderRadius: 10,
                    padding: '12px 14px',
                    font: 'inherit',
                  }}
                ...
                {errors.confirm_password && (
                  <span
                    role="alert"
                    style={{ color: C.red, display: 'block', marginTop: 5, fontSize: 12 }}
                  >
              ...
              {errors.accept_terms && (
                <span
                  role="alert"
                  style={{ color: C.red, display: 'block', margin: '-12px 0 14px', fontSize: 12 }}
                >
                  {errors.accept_terms.message}
                </span>
              )}
              <p style={{ color: C.t4, fontSize: 12, lineHeight: 1.5, marginTop: -8 }}>
          ...
          <Btn full size="lg" disabled={mutation.isPending}>
        ...
        {mode === 'register' && (
          <p style={{ color: C.t4, fontSize: 13, textAlign: 'center', marginTop: 16 }}>
            Join 2,400+ explorers discovering what fits
          </p>
        )}
        <button
          onClick={() => setScreen(mode === 'login' ? 'register' : 'login')}
          style={{
            width: '100%',
            marginTop: 14,
            color: C.acc,
