      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.t4 }}>
              {active?.experiment.title ?? 'Active experiment'} — Day {active?.current_day ?? 1}
            </span>
      ...
      >
        ...
        {current.type === 'yn' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { label: 'Yes, I completed it', v: 1, color: C.acc },
              { label: 'Partially completed', v: 2, color: C.amber },
              { label: 'Not today', v: 0, color: C.t3 },
            ].map(({ label, v, color }) => (
              <button
                key={v}
                onClick={() => select(v)}
                style={{
                  padding: '18px',
                  borderRadius: 12,
                  border: `1px solid ${answers[step] === v ? `${color}55` : C.br}`,
        ...
        {current.type === 'note' && (
          <div>
            ...
            />
            <div style={{ fontSize: 12, color: C.t4, marginTop: 6, textAlign: 'right' }}>
              {notes.length} / 500
            </div>
