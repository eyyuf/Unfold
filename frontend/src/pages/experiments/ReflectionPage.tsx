  ...
  return (
    ...
    >
      <Card style={{ width: '100%', maxWidth: 560, padding: 32 }}>
        ...
        <p style={{ color: C.t3, lineHeight: 1.6 }}>
          Your response becomes part of your evidence report. It is a clue, not a verdict.
        </p>
        <label style={{ display: 'block', margin: '24px 0 10px', color: C.t2 }}>
          Would you choose to continue this activity?
        </label>
        <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
          {[1, 2, 3, 4, 5].map((value) => (
            <button
              key={value}
              onClick={() => setRepeatIntent(value)}
              style={{
                flex: 1,
                padding: 14,
                borderRadius: 10,
                border: `1px solid ${value === repeatIntent ? C.acc : C.br}`,
                background: value === repeatIntent ? C.accS : C.s2,
                color: value === repeatIntent ? C.acc : C.t2,
                cursor: 'pointer',
              }}
            >
              {value}
        ...
        >
          {submit.isPending ? 'Creating report…' : 'Complete and view report'}
        </Btn>
      </Card>
    </div>
  )
