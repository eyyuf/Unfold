...
export default function ExperimentsPage({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  const saveExperiment = useMutation({
    mutationFn: ({ slug, saved }: { slug: string; saved: boolean }) =>
      experimentService.toggleSaved(slug, saved),
  ...
  const categoryStyle: Record<string, { color: string; Icon: React.ElementType }> = {
    Creative: { color: C.purple, Icon: Star },
    Technical: { color: C.blue, Icon: Brain },
    Social: { color: C.orange, Icon: Users },
    Nature: { color: C.acc, Icon: Leaf },
    Service: { color: C.teal, Icon: Heart },
    Physical: { color: C.amber, Icon: Dumbbell },
  }
  ...
  const exps = data
    .map((item) => ({
      ...
      mins: item.minutes_per_day,
      desc: item.description,
      ...(categoryStyle[item.category] ?? { color: C.acc, Icon: Compass }),
    }))
  ...
  if (isLoading)
    return (
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 24px' }}>
        <div
          className="skeleton"
          style={{ width: 200, height: 28, borderRadius: 8, marginBottom: 28 }}
        />
        <div
          style={{
            display: 'grid',
  ...
  if (error)
    return (
      <ErrorBlock message="The experiment library could not be loaded." onRetry={() => refetch()} />
    )
    ...
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 24px' }} className="fade-up">
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 15, color: C.t3 }}>
            Pick one that feels worth exploring. You can always try another after.
          </p>
        </div>
        {user && (
          <Btn variant="ghost" size="sm" onClick={() => setScreen('saved')}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: C.t4, letterSpacing: '0.05em' }}>
            RECOMMENDED FOR YOU
          </span>
        </div>
        <div
          ...
          {exps.slice(0, 2).map(({ title, slug, cat, color, Icon, days, mins, desc, badge }) => (
            <Card key={title} accent onClick={() => openExperiment(slug)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <button
                  aria-label={`${savedSlugs.has(slug) ? 'Remove' : 'Save'} ${title}`}
                  onClick={(event) => toggleSave(event, slug)}
                  style={{
                    background: 'none',
                    border: 'none',
                    cursor: 'pointer',
                    color: savedSlugs.has(slug) ? C.acc : C.t4,
                  }}
                >
                  <Bookmark size={15} fill={savedSlugs.has(slug) ? 'currentColor' : 'none'} />
                </button>
              </div>
              {badge && (
                <div
                ...
                >
                  <Sparkles size={12} /> {badge.toUpperCase()}
                </div>
              )}
              <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, lineHeight: 1.3 }}>
                {title}
              </h3>
              <p style={{ fontSize: 13, color: C.t3, lineHeight: 1.55, marginBottom: 14 }}>
                {desc}
              </p>
              <div
        ...
        >
          {exps.map(({ title, slug, cat, color, Icon, days, mins, desc }) => (
            <Card key={title} onClick={() => openExperiment(slug)}>
              ...
              <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 8, lineHeight: 1.3 }}>
                {title}
              </h3>
              <p style={{ fontSize: 13, color: C.t3, lineHeight: 1.55, marginBottom: 14 }}>
                {desc}
