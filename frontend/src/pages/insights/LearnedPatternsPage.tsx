  ...
  return (
    <div style={{ maxWidth: 840, margin: '0 auto', padding: '32px 24px' }} className="fade-up">
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        {!uncertain.length && (
          <Card style={{ color: C.t4, fontSize: 14 }}>No uncertain hypotheses at present.</Card>
        )}
      </div>
      ...
      {contradicted.length > 0 && (
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ color: C.t4, fontSize: 13, lineHeight: 1.55, marginBottom: 14 }}>
          ...
          {detailLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, margin: '12px 0' }}>
              {detailData?.evidence.map((ev, i) => (
                ...
                >
                  <div style={{ fontWeight: 700, color: C.t1, marginBottom: 4 }}>
                    {ev.experiment_title}
                  </div>
                  <div style={{ display: 'flex', gap: 12, color: C.t3 }}>
                    <span>
                      Fit score:{' '}
                      <strong style={{ color: C.acc }}>{Math.round(ev.fit_score)}%</strong>
                    </span>
                    <span>
                      Confidence: <strong>{Math.round(ev.confidence_score)}%</strong>
                    </span>
                    <span>
                      Trait weight: <strong>{ev.weight}/5</strong>
                    </span>
                  </div>
                </div>
              ))}
              {!detailData?.evidence?.length && (
                <p style={{ color: C.t4 }}>No supporting evidence records found.</p>
              )}
            </div>
          )}
        </ConfirmModal>
          ...
          {testLoading ? (
            ...
            <div style={{ margin: '12px 0' }}>
              <Card accent style={{ marginBottom: 14 }}>
                ...
                <h3 style={{ fontSize: 18, fontWeight: 700, margin: '8px 0 6px' }}>
