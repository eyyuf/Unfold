...
export default function ProfilePage({ setScreen }: { setScreen: (s: Screen) => void }) {
  const [theme, setTheme] = useState<ThemePreference>(
    () => (localStorage.getItem('unfold-theme') as ThemePreference | null) ?? 'dark',
  )
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px' }} className="fade-up">
      <h1 style={{ fontSize: 26, fontWeight: 800, marginBottom: 28, letterSpacing: '-0.02em' }}>
        Profile & settings
      </h1>
      ...
      <Card style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20 }}>
          <div
            style={{
              ...
              height: 56,
              borderRadius: '50%',
              background: `linear-gradient(135deg, ${C.purple}44, ${C.blue}44)`,
              display: 'flex',
              alignItems: 'center',
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: '1fr 130px', gap: 10, marginBottom: 16 }}
        >
          <input
        ...
        >
          ...
          <select
            ...
            onChange={(event) => {
              ...
              updateProfile.mutate({ timezone: event.target.value })
            }}
            style={{
              background: C.s2,
              color: C.t1,
      ...
      <Card style={{ marginBottom: 20 }}>
        ...
        {[
          {
            ...
            active: Boolean(user?.reminders_enabled),
          },
          {
            label: 'Send by email',
            field: 'email_reminders_enabled',
            active: Boolean(user?.email_reminders_enabled),
          },
          ...
          >
            ...
            <button
              ...
              aria-checked={active}
              aria-label={label}
              onClick={() => updateProfile.mutate({ [field]: !active })}
              style={{
                width: 44,
                height: 24,
                padding: 0,
        ...
        >
          ...
          <input
            ...
            onChange={(event) => {
              ...
              updateProfile.mutate({ reminder_time: event.target.value })
            }}
            style={{
              background: C.s2,
              border: `1px solid ${C.br}`,
        ...
        <p style={{ color: C.t4, fontSize: 12, margin: '12px 0 0' }}>
          Reminders use {timezone}. A preview: “Today’s experiment is ready when you are.”
        </p>
        {updateProfile.error && (
          ...
          {(
            ...
            <button
              ...
              style={{
                ...
                padding: '10px',
                borderRadius: 10,
                border: `1px solid ${theme === id ? C.accB : C.br}`,
                background: theme === id ? C.accS : C.s1,
                color: theme === id ? C.acc : C.t3,
                fontFamily: 'inherit',
                fontSize: 13,
                fontWeight: 600,
                cursor: 'pointer',
      ...
      <Card style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          >
            <span
              style={{ display: 'flex', alignItems: 'center', gap: 10, color: C.t2, fontSize: 14 }}
            >
              <BarChart2 size={15} color={C.t4} />
              Optional analytics
            </span>
            <button
              role="switch"
              aria-checked={Boolean(user?.analytics_consent)}
              aria-label="Optional analytics consent"
              onClick={() => updateProfile.mutate({ analytics_consent: !user?.analytics_consent })}
              style={{
                width: 44,
                height: 24,
                padding: 0,
                border: 0,
                borderRadius: 12,
                background: user?.analytics_consent ? C.acc : C.bg2,
                position: 'relative',
                cursor: 'pointer',
              }}
            >
              <span
                style={{
                  position: 'absolute',
                  top: 3,
                  left: user?.analytics_consent ? 23 : 3,
                  width: 18,
                  height: 18,
                  borderRadius: '50%',
                  background: C.t1,
          ...
          <button
            onClick={() => setShowConsents((visible) => !visible)}
            style={{
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <Shield size={15} color={C.t4} />
              View consent history
            </span>
            <ChevronRight
              size={14}
              color={C.t4}
              style={{ transform: showConsents ? 'rotate(90deg)' : undefined }}
            />
          </button>
          {showConsents && (
            <div style={{ background: C.bg2, borderRadius: 10, padding: '4px 14px' }}>
              {!consents.length && (
                <p style={{ color: C.t4, fontSize: 13 }}>No consent records found.</p>
              )}
              {consents.map((record) => (
                <div
                  key={record.id}
                ...
                >
                  <span style={{ color: C.t2 }}>
                    {record.kind}: {record.granted ? 'Granted' : 'Declined'}
                  </span>
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <Download size={15} color={C.t4} />
              {exportData.isPending ? 'Preparing PDF…' : 'Export as PDF'}
            </span>
          ...
          {(exportData.error || deleteUser.error) && (
            <p role="alert" style={{ color: C.red, fontSize: 13 }}>
              {(exportData.error || deleteUser.error)?.message}
            </p>
          )}
        </div>
      </Card>
      <Btn variant="ghost" full disabled={logoutUser.isPending} onClick={() => logoutUser.mutate()}>
        Log out
      </Btn>
        ...
        <label style={{ display: 'block', color: C.t2, fontSize: 14 }}>
          ...
          <input
            ...
            style={{
              ...
              background: C.s2,
              color: C.t1,
              border: `1px solid ${deleteConfirmation === 'DELETE' ? C.red : C.br}`,
              borderRadius: 10,
              padding: '12px 14px',
              font: 'inherit',
            }}
