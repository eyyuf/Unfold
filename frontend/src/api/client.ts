...
export async function apiRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new Error(navigator.onLine ? "The service could not be reached. Please try again." : "You appear to be offline. Your progress is still on this device.")
  }
  if (!response.ok) {
    ...
    const firstError = payload && typeof payload === "object"
      ...
      : null
    throw new Error(payload?.detail ?? firstError ?? "The request could not be completed. Please try again.")
  }
