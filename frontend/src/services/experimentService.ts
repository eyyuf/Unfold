...
import type { ActiveExperiment, ExperimentData, ExperimentReport, SavedExperimentData } from '@/types'
...
export const experimentService = {
  ...
  start: (slug: string, input: StartExperimentInput) =>
    apiRequest<ActiveExperiment>(`/experiments/${slug}/start/`, { method: 'POST', body: JSON.stringify(input) }),
  startCheckIn: (id: number, dayNumber: number, motivationBefore: number) =>
    apiRequest(`/user-experiments/${id}/checkins/start/`, { method: 'POST', body: JSON.stringify({ day_number: dayNumber, motivation_before: motivationBefore }) }),
  submitCheckIn: (id: number, input: CheckInInput) =>
    apiRequest(`/user-experiments/${id}/checkins/`, { method: 'POST', body: JSON.stringify(input) }),
  completeReflection: (id: number, repeatIntent: number, summary: string) =>
    apiRequest(`/user-experiments/${id}/final-reflection/`, { method: 'POST', body: JSON.stringify({ repeat_intent: repeatIntent, summary }) }),
  abandon: (id: number) => apiRequest(`/user-experiments/${id}/abandon/`, { method: 'POST' }),
  getReport: (id: string | number) => apiRequest<ExperimentReport>(`/user-experiments/${id}/report/`),
