...
export function exportEvidencePdf(data: Record<string, any>) {
  ...
  doc.text(
    `Exported ${new Date(data.exported_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`,
    W / 2,
    182,
    { align: 'center' },
  )
  ...
  doc.setTextColor(161, 161, 170)
  doc.text(
    `${completed.length} experiments completed  •  ${experiments.reduce((s: number, e: any) => s + (e.checkin_count || 0), 0)} total check-ins`,
    W / 2,
    200,
    { align: 'center' },
  )
  ...
  experiments.forEach((exp: any, idx: number) => {
    ...
    doc.setTextColor(161, 161, 170)
    const meta = [
      exp.experiment?.category,
      `${exp.experiment?.duration_days || '?'} days`,
      exp.status,
    ]
      .filter(Boolean)
      .join('  •  ')
    ...
    const fitColor: [number, number, number] =
      fit >= 70 ? [34, 197, 94] : fit >= 45 ? [245, 158, 11] : [161, 161, 170]
    ...
    if (exp.dimensions) {
      ...
      Object.entries(exp.dimensions as Record<string, number>).forEach(([dim, val]) => {
        ...
        const barColor: [number, number, number] =
          val >= 70 ? [34, 197, 94] : val >= 45 ? [245, 158, 11] : [161, 161, 170]
