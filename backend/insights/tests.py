...
from experiments.models import Category, Experiment, ExperimentTrait, ExperimentTraitWeight, UserExperiment
...
class ScoringServiceTests(TestCase):
    ...
    def test_confidence_score_range(self):
        ...
        exp = Experiment.objects.create(category=cat, title="Exp", slug="exp", description="Desc", duration_days=7, minutes_per_day=20, published=True)
        ue = UserExperiment.objects.create(user=user, experiment=exp, start_date="2026-08-01", status="active")
        ...
        for d in range(1, 8):
            CheckIn.objects.create(user_experiment=ue, day=d, energy_after=4, curiosity=4, meaning=4, is_complete=True)
        ...
        ue.status = "completed"
        ue.save()
        FinalReflection.objects.create(user_experiment=ue, repeat_intent=5, summary="Great")
...
class TraitEvidenceAndHypothesisTests(TestCase):
    def setUp(self):
        ...
        self.trait_creative = ExperimentTrait.objects.create(
            slug="creative", name="Creative", positive_hypothesis_text="Creative activities suit you."
        )
        self.trait_tangible = ExperimentTrait.objects.create(
            slug="tangible_output", name="Tangible Output", positive_hypothesis_text="You like tangible output."
        )

        self.exp1 = Experiment.objects.create(category=self.cat, title="Exp 1", slug="exp-1", description="D", duration_days=3, minutes_per_day=20, published=True)
        ExperimentTraitWeight.objects.create(experiment=self.exp1, trait=self.trait_creative, weight=5)
        ExperimentTraitWeight.objects.create(experiment=self.exp1, trait=self.trait_tangible, weight=4)

        self.exp2 = Experiment.objects.create(category=self.cat, title="Exp 2", slug="exp-2", description="D", duration_days=3, minutes_per_day=20, published=True)
        ExperimentTraitWeight.objects.create(experiment=self.exp2, trait=self.trait_creative, weight=5)

    def test_trait_evidence_generation_is_idempotent(self):
        ue = UserExperiment.objects.create(user=self.user_a, experiment=self.exp1, start_date="2026-08-01", status="completed")
        for d in range(1, 4):
            CheckIn.objects.create(user_experiment=ue, day=d, enjoyment=5, energy_after=5, curiosity=5, meaning=5, is_complete=True)
        FinalReflection.objects.create(user_experiment=ue, repeat_intent=5, summary="Loved it")
    ...
    def test_hypothesis_status_progression(self):
        ...
        ue1 = UserExperiment.objects.create(user=self.user_a, experiment=self.exp1, start_date="2026-08-01", status="completed")
        for d in range(1, 4):
            CheckIn.objects.create(user_experiment=ue1, day=d, enjoyment=5, energy_after=5, curiosity=5, meaning=5, is_complete=True)
        FinalReflection.objects.create(user_experiment=ue1, repeat_intent=5, summary="S1")
        generate_trait_evidence(ue1)

        hyps1 = recalculate_user_hypotheses(self.user_a)
        hyp_creative = UserHypothesis.objects.get(user=self.user_a, trait=self.trait_creative)
        self.assertEqual(hyp_creative.status, UserHypothesis.Status.UNCERTAIN)

        # 2 experiments with high fit -> emerging
        ue2 = UserExperiment.objects.create(user=self.user_a, experiment=self.exp2, start_date="2026-08-05", status="completed")
        for d in range(1, 4):
            CheckIn.objects.create(user_experiment=ue2, day=d, enjoyment=5, energy_after=5, curiosity=5, meaning=5, is_complete=True)
        FinalReflection.objects.create(user_experiment=ue2, repeat_intent=5, summary="S2")
...
class PermissionAndEndpointTests(TestCase):
    def setUp(self):
        ...
        self.exp = Experiment.objects.create(category=self.cat, title="Exp", slug="exp", description="Desc", duration_days=3, minutes_per_day=20, published=True)
        self.ue_a = UserExperiment.objects.create(user=self.user_a, experiment=self.exp, start_date="2026-08-01", status="completed")
        self.ue_b = UserExperiment.objects.create(user=self.user_b, experiment=self.exp, start_date="2026-08-01", status="completed")

        self.trait = ExperimentTrait.objects.create(slug="trait-a", name="Trait A")
        self.hyp_a = UserHypothesis.objects.create(user=self.user_a, trait=self.trait, support_score=80, confidence_score=75, status="supported", evidence_count=3)
        self.hyp_b = UserHypothesis.objects.create(user=self.user_b, trait=self.trait, support_score=80, confidence_score=75, status="supported", evidence_count=3)
