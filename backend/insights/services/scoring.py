...
def calculate_daily_fit(checkin: Any, expected_minutes: int = 20) -> float:
    ...
    if interest_vals:
        interest_score = sum(scale_to_100(v) for v in interest_vals) / len(interest_vals)
    else:
        interest_score = 50.0

    energy_score = scale_to_100(checkin.energy_after) if checkin.energy_after is not None else scale_to_100(checkin.energy)
    meaning_score = scale_to_100(checkin.meaning)
    flow_score = scale_to_100(checkin.lost_track_of_time) if checkin.lost_track_of_time is not None else 50.0
    satisfaction_score = scale_to_100(checkin.satisfaction_after) if checkin.satisfaction_after is not None else interest_score

    minutes = checkin.minutes_spent if checkin.minutes_spent is not None else expected_minutes
    completion_ratio = min(minutes / max(1, expected_minutes), 1.0) if (checkin.is_complete or minutes > 0) else 0.0
...
def calculate_evidence_confidence(user_experiment: Any) -> Tuple[float, str]:
    ...
    if hasattr(user_experiment, "final_reflection") and user_experiment.final_reflection is not None:
        confidence += 25.0
...
def calculate_overall_fit(user_experiment: Any) -> float:
    ...
    completed_count = user_experiment.checkins.filter(is_complete=True).count() or len(checkins)
    consistency = min(100.0, (completed_count / expected_days) * 100.0)

    has_reflection = hasattr(user_experiment, "final_reflection") and user_experiment.final_reflection is not None
    if has_reflection:
        repeat_intent = user_experiment.final_reflection.repeat_intent
        final_reflection_score = scale_to_100(repeat_intent)
        overall = (
            0.60 * avg_daily_fit
            + 0.20 * final_reflection_score
            + 0.20 * consistency
        )
    else:
        overall = (
...
def calculate_before_after_delta(motivation_before: Optional[int], satisfaction_after: Optional[int]) -> Dict[str, Any]:
    """Calculate motivation vs satisfaction delta and interpretation."""
    if motivation_before is None or satisfaction_after is None:
        return {
            "motivation_before": scale_to_100(motivation_before) if motivation_before else None,
            "satisfaction_after": scale_to_100(satisfaction_after) if satisfaction_after else None,
            "delta": 0,
    ...
    if delta >= 2:
        interpretation = "Starting was difficult, but you felt much better after doing it."
    elif delta == 1:
        ...
        interpretation = "You were more excited before the activity than after completing it."
