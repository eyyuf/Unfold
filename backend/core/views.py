    ...
    return Response(
        {
            ...
            "days": [
                {"date": activity_date.isoformat(), "count": counts[activity_date]}
                for activity_date in ordered_dates
            ],
...
class ExperimentList(generics.ListAPIView):
    ...
    def get_queryset(self):
        qs = (
            Experiment.objects.filter(published=True)
            .select_related("category")
            .prefetch_related("daily_tasks")
        )
        ...
        if os.environ.get("RESEND_API_KEY"):
            try:
                ...
                resend.Emails.send(
                    {
                        "from": os.environ.get(
                            "DEFAULT_FROM_EMAIL", "Unfold <onboarding@resend.dev>"
                        ),
                        "to": [user.email],
...
def me_view(request):
    ...
    if request.method == "PATCH":
        ...
        serializer.save()
        if (
            "analytics_consent" in request.data
            and request.user.analytics_consent != previous_analytics_consent
        ):
...
def export_user_data(request):
    experiments = (
        UserExperiment.objects.filter(user=request.user)
        .select_related("experiment__category")
        .prefetch_related("checkins")
    )
    saved = (
        SavedExperiment.objects.filter(user=request.user)
        .select_related("experiment__category")
        .prefetch_related("experiment__daily_tasks")
    )
    return Response(
        {
...
def start_experiment(request, slug):
    ...
    existing = (
        UserExperiment.objects.filter(user=request.user, status="active")
        .select_related("experiment__category")
        .first()
    )
    ...
    return Response(
        UserExperimentSerializer(active).data,
...
def toggle_saved_experiment(request, slug):
    ...
    if request.method == "DELETE":
        SavedExperiment.objects.filter(
            user=request.user, experiment=experiment
        ).delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
    saved, created = SavedExperiment.objects.get_or_create(
        user=request.user, experiment=experiment
    )
    return Response(
        SavedExperimentSerializer(saved).data,
        status=status.HTTP_201_CREATED if created else status.HTTP_200_OK,
    )
...
def active_experiment(request):
    item = (
        UserExperiment.objects.filter(user=request.user, status="active")
        .select_related("experiment__category")
        .prefetch_related("checkins", "experiment__daily_tasks")
        .first()
    )
    if not item:
        return JsonResponse(None, safe=False)
...
def final_reflection(request, pk):
    ...
    reflection, _ = FinalReflection.objects.update_or_create(
        user_experiment=item, defaults=serializer.validated_data
    )
...
def report_for(item):
    ...
    avg_get = lambda attr: (
        round(sum(getattr(row, attr) or 3 for row in checkins) / count * 20)
        if count
        else 0
    )
    consistency = min(100, round(count / max(1, item.experiment.duration_days) * 100))
    repeat_intent = scale_to_100(
        getattr(getattr(item, "final_reflection", None), "repeat_intent", None)
    )

    dimensions = {
        "Energy": (
            avg_get("energy_after")
            if any(c.energy_after for c in checkins)
            else avg_get("energy")
        ),
        "Curiosity": avg_get("curiosity"),
        "Meaning": avg_get("meaning"),
        "Ease": (
            round(sum(scale_to_100(6 - (c.difficulty or 3)) for c in checkins) / count)
            if count
            else 0
        ),
    ...
    strongest = (
        max(dimensions, key=dimensions.get) if populated else "Not enough evidence"
    )
...
def user_hypotheses_list(request):
    recalculate_user_hypotheses(request.user)
    hypotheses = UserHypothesis.objects.filter(user=request.user).select_related(
        "trait"
    )
...
def user_hypothesis_detail(request, pk):
    ...
    evidence_list = TraitEvidence.objects.filter(
        user=request.user, trait=hyp.trait
    ).select_related("user_experiment__experiment")
    evidence_data = [
        {
...
def test_hypothesis(request, pk):
    ...
    if not rec:
        return Response(
            {"detail": "No suitable test experiment found for this hypothesis."},
...
def patterns_list(request):
    patterns = PatternDefinition.objects.filter(is_active=True).prefetch_related(
        "traits"
    )
    matched_patterns = []
    for pat in patterns:
        ...
        qualifying_runs = (
            ...
            .annotate(
                matched_count=Count(
                    "experiment__trait_weights__trait_id", distinct=True
                )
            )
            .filter(matched_count=len(required_trait_ids))
        )
...
def insights_view(request):
    recalculate_user_hypotheses(request.user)
    items = (
        UserExperiment.objects.filter(user=request.user, status="completed")
        .select_related("experiment__category")
        .prefetch_related("checkins")
    )
    ...
    average_curiosity = (
        round(sum(row.curiosity or 3 for row in checkins) / len(checkins), 1)
        if checkins
        else 0
    )
    average_repeat_intent = (
        round(sum(row.repeat_intent for row in reflections) / len(reflections) * 20)
        if reflections
        else 0
    )
    average_consistency = (
        round(
            sum(report["dimensions"]["Consistency"] for report in reports)
            / len(reports)
        )
        if reports
        else 0
    ...
    for hypothesis in hypotheses.order_by("-confidence_score", "-support_score"):
        if hypothesis.status not in (
            UserHypothesis.Status.SUPPORTED,
            UserHypothesis.Status.EMERGING,
        ):
            continue
    ...
    return Response(
        {
            ...
            "average_fit": (
                round(sum(r["fit_signal"] for r in reports) / len(reports))
                if reports
                else 0
            ),
            ...
            "patterns": [
                f"{report['strongest_signal']} is your strongest signal in {report['experiment']['title']}"
