...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        active_items = UserExperiment.objects.filter(
            status="active",
            user__reminders_enabled=True,
            user__email_reminders_enabled=True,
        ...
        for item in active_items:
            ...
            target = datetime.datetime.combine(
                local_now.date(), item.user.reminder_time, tzinfo=local_now.tzinfo
            )
            ...
            try:
                resend.Emails.send(
                    {
                        "from": os.environ.get(
                            "DEFAULT_FROM_EMAIL", "Unfold <hello@example.com>"
                        ),
                        "to": item.user.email,
