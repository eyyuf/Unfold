...
class DemoSeedCommandTests(TestCase):
    ...
    @patch.dict("os.environ", {"UNFOLD_DEMO_PASSWORD": "demo-test-password", "UNFOLD_DEMO_EMAIL": "demo@example.com"})
    def test_seed_is_repeatable_and_builds_complete_demo_story(self):
        ...
        self.assertEqual(UserExperiment.objects.filter(user=user, status="completed").count(), 3)
        active = UserExperiment.objects.get(user=user, status="active")
        self.assertEqual(active.checkins.filter(is_complete=True).count(), active.experiment.duration_days - 1)
        self.assertEqual(CheckIn.objects.filter(user_experiment__user=user).count(), 32)
        self.assertTrue(UserHypothesis.objects.filter(user=user, status="supported").exists())
